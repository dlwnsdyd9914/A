...
final class TweetCell: UICollectionViewCell {
    ...
    override func prepareForReuse() {
        ...
        viewModel?.onTweetLikes           = nil
        viewModel?.ondidLike              = nil
        viewModel?.onHandleCommentButton  = nil   // ✅
        viewModel?.onMentionUserFetched   = nil   // ✅
        viewModel = nil
        onProfileImageViewTapped = nil
        onCommentButtonTapped    = nil
    ...
    private func bindViewModel(viewModel: TweetViewModel) {
        DispatchQueue.main.async { [weak self, weak vm = viewModel] in
            ...
            self.infoLabel.attributedText = vm.infoLabel
            self.captionLabel.text = vm.caption
            self.profileImageView.kf.setImage(with: vm.profileImageUrl)
            self.replyLabel.isHidden = vm.shouldHideReplyLabel
            self.replyLabel.text = vm.replyText
        }


        viewModel.onTweetLikes = { [weak self, weak vm = viewModel] in
            guard let self, let vm else { return }
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                self.likeButton.setImage(vm.likeButtonImage, for: .normal)
            }
        }
    ...
    private func conigureMentionHandler() {
        captionLabel.handleMentionTap { [weak vm = viewModel] username in
            guard let vm else { return }
            vm.handleMention(username: username)
        }
    }
}
